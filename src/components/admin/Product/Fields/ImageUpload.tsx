...
type Prop = {
  ...
  callbackUpload: (keys: string, value: File) => void;
  callbackDelete: () => void;
  files: File[] | [];
}

export default function ImageUpload(props: Prop) {
  const { 
    loader,
    mainkeyTarget, 
    initialValue, 
  ...
  const media        = useRef<HTMLDivElement>(null); // images/media wrapper
  ...
  async function fileHandler(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (!files) { return 'no data to be saved'; } 
    ...
    try {
      ...
      callbackUpload(keys, compressedFile)
    } catch (error) {
  ...
  return (
    <div className={`form__field ${loader ? 'form__field--skeleton' : ''}`}>
      ...
      <div className="form__field-media form__field-media--single" ref={media}>
        {
          initialValue && initialValue.length > 0 ? 
            <div  className="form__field-img" >
              <span className="form__field-img-delete" onClick={deleteImage}>
              ...
              <img src={initialValue[0].url} alt={initialValue[0].name} />
            </div>
          :
            <label>
              ...
              <input
                ...
                name={mainkeyTarget} 
                id={uid}
                accept="image/png, image/jpeg, image/webp, image/avif"></input>
            </label>
        }
      </div>
    </div>
