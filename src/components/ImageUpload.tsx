...
// This way if the user accidentally refresh or closed the app, those untracked
// images will not be stored in the bucket.
...
export default function ImageUpload(props: ImageUpload) {
      ...
      try {
        ...
        setProduct(prev => {
          ...
          const newImage = {
            ...
            sort_order_id: prev.images.length,
            url: URL.createObjectURL(compressedFile),
            size: compressedFile.size,
            type: compressedFile.type,
          }

          const files = prev.files ? [...prev.files, compressedFile] : [compressedFile]
  ...
  function deleteImage(image: Image) {
    setProduct(prev => {
      ...
      const newArr = []
      const filterImages = prev.images.filter(i => i.sort_order_id !== image.sort_order_id)
    ...
    if (product.images.length === 0 && upload && upload.current) {
  ...
  const handleDragDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    for (let i = 0; i < product.images.length; i++) {
      ...
      if(product.files && product.files.length > 0) {
        if (product.files[i]) {
          newImagesFileArrangement.push(product.files[i])
        }
      }
    }
    ...
    setProduct(prev => {
      ...
      return {
        ...prev,
        images: newImages,
        files: newImagesFileArrangement
      }
    });
  ...
  return (
    <div className={`form__field ${props.dynamic && isLoading ? 'form__field--skeleton' : ''}`}>
      ...
      <div className="form__field-media" ref={media}>
        ...
        <div className={`${product && product.images.length > 0 ? 'form__field-img' : ''}`}>
          <label>
